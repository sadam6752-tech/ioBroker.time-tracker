...
Sperrzeit), die Automatik in `automation_rules`/`automation_runs` (Ausstempeln, Meldung, Pausenerinnerung — innerhalb
der gewählten Wochentage, höchstens einmal pro Mitarbeiter und Tag bzw. ISO-Woche) sowie `sendTo`-Nachrichten (`punch`,
`present`, `status`, `report`, `backup`). Stempel selbst bleiben in der Datenbank.
