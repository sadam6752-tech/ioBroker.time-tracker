...
| T7 | Berichte | Excel und PDF für Monat **und** Jahr erzeugen; danach als Administrator im Reiter *Berichte* einen **Mitarbeiter auswählen** und dessen Excel/PDF laden sowie über den Monatsnamen dessen Monatsansicht öffnen | beide laden, Dateiname mit Zeitraum, Zahlen identisch zur Monatsansicht; die Datei des Mitarbeiters trägt **dessen** Login im Namen und seine Monatsansicht zeigt dessen Tage; ohne Unicode-Schrift in `ru`/`uk`/`zh-cn` → **422 `report_font_missing`** mit Hinweis auf `report_font_path` |
